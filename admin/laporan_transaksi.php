...
$to = $_POST["to"];
    ...
    $result = '';
    $query = "SELECT * FROM qw_transaksi WHERE tgl_sewa BETWEEN '".$_POST["From"]."' AND '".$_POST["to"]."' and status_transaksi = 'Selesai'";
...
<td>' .$row["status_transaksi"].' </td>
    ...
    $total_pemasukan = mysql_query("select SUM(nominal_bayar)+sum(denda) as total_pemasukan from qw_transaksi WHERE tgl_sewa BETWEEN '".$_POST["From"]."' AND '".$_POST["to"]."' and status_transaksi = 'Selesai'");
$data = mysql_fetch_array($total_pemasukan);
    $result .='<tr><td colspan="6">Total Pemasukan (Nominal bayar + Denda)</td>
    <td colspan="4">Rp. '.$data['total_pemasukan'].'</td></tr></table>
    <br>
